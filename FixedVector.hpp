...
class FixedVector
{
  ...
  private:
    ...
    std::size_t const _capacity = 0;                                      // length of the array
...
template<typename T>
FixedVector<T>::FixedVector( std::size_t capacity )
...
template <typename T>
void FixedVector<T>::set( std::size_t index, const T & value )
...
template <typename T>
std::size_t FixedVector<T>::erase( std::size_t index )
{
  if( index >= _size ) throw std::range_error( "index out of bounds" );
...
template <typename T>
T * FixedVector<T>::erase( T * position )
{
...
template <typename T>
std::size_t FixedVector<T>::insert( std::size_t beforeIndex, const T & value )
{
  if( _size       >= _capacity ) throw std::range_error( "insufficient capacity to add another element" );
...
template <typename T>
T* FixedVector<T>::insert( T * beforePosition, const T & value )
{
...
// Copy Constructor
template <typename T>
FixedVector<T>::FixedVector( const FixedVector<T> & other )
...
FixedVector<T> & FixedVector<T>::operator=( const FixedVector<T> & rhs )
{
  if( this != &rhs )
  {
    // Being fixed size, the already allocated array can be reused. Capacity is not adjusted.  If _capacity < rhs._capacity, then some
    // rhs elements may not be copied and the vector is truncated.
